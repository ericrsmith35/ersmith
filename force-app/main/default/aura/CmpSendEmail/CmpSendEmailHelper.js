            ...
            for(var i = 0; i < params.length; i++)
            {
                switch (params[i])
                {
                    ...
                    case 'lstFiles':
                        ...
                        break;
                    case 'fromEmail':
                        component.set("v.fromEmail", pageReference.state[params[i]]);
                        var oweas = component.get("v.lstFrom");
                        for(var pl = 0 ; pl < oweas.length; pl ++){
                            if(oweas[pl].label.includes(pageReference.state[params[i]])){
                                component.set("v.fromId", oweas[pl].value);
                            }
                        }
                        break;
                }
